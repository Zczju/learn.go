...
import (
	"fmt"
	c02 "learn.go/chapter02/12.15_refactor_fatrateCalc/calc"
	calculator "learn.go/chapter02/12.15_refactor_fatrateCalc/calc"
)
...
func mainFatRateBody() {
	...
	if err != nil {
		fmt.Println("Warning: Failed to calculate fatRate: ", err)
		return
	}
...
func calcFatRate(weight float64, tall float64, age int, sex string) (fatRate float64, err error) {
	// 计算体脂率
	bmi, err := calculator.CalcBMI(tall, weight)
	if err != nil {
		return 0, err
	}
