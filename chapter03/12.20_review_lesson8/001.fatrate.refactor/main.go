...
import (
	"fmt"
	calculator2 "learn.go/chapter03/12.20_review_lesson8/001.fatrate.refactor/calc"
)
...
func mainFatRateBody() {
	...
	fatRate, _ := calcFatRate(weight, tall, age, sex)
	if fatRate <= 0 {
...
func calcFatRate(weight float64, tall float64, age int, sex string) (fatRate float64, err error) {
	// 计算体脂率
	bmi, _ := calculator2.CalcBMI(tall, weight)
	fatRate = calculator2.CalcFatRate(bmi, age, sex)
